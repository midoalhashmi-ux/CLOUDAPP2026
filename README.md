...
## خطوة لا بد منها قبل أول بناء ناجح
هذه الملفات تفترض وجود `firebase_options.dart` (يُولَّد تلقائياً، لا يُكتب يدوياً):
1. ثبّت Firebase CLI و FlutterFire CLI محلياً أو استخدم بيئة تدعمها.
2. نفّذ: `flutterfire configure` واربطه بمشروع Firebase الخاص بك.
3. ضع الملف الناتج في `lib/firebase_options.dart`.
4. في `lib/main.dart`، أزل التعليق عن السطرين المتعلقين بـ
   `firebase_options.dart` و `DefaultFirebaseOptions.currentPlatform`.

بدون هذه الخطوة، التطبيق سيفشل بالبناء أو بالتشغيل عند استدعاء `Firebase.initializeApp`.
