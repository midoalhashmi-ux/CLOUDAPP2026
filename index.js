...
function normalizeFixture(fx) {
  ...
  return {
    ...
    // اسم الدولة كما يرجعه API-Football — ضروري للتفريق بين دوريات
    // بنفس الاسم بالضبط في دول مختلفة (مثال حقيقي: "Premier League"
    // إنجليزي مقابل "Premier League" هونغ كونغي)، راجع
    // FootballTranslations.isSupportedLeague في تطبيق فلاتر.
...
//
// الحصيلة: 24 طلب/يوم (اليوم كل ساعة) + 24 طلب/يوم إضافية (غداً كل ساعة
// أيضاً، انظر السبب أدناه) + ~3 طلبات فقط مرة واحدة يومياً (يومان قادمان
// إضافيان + يوم ماضٍ واحد جديد) ≈ 51 طلب/يوم، ضمن حد الخطة المجانية
// (100 طلب/يوم) بهامش مريح.
async function runScheduledSync(env) {
  await fetchAndStoreFixtures(env, dateKeyOffset(0));

  // "غداً" (offset +1) يُجلب كل ساعة مثل اليوم بالضبط — وليس مرة واحدة
  // يومياً عند الساعة 0 فقط كما كان سابقاً. السبب: لو نُشر هذا الووركر
  // لأول مرة، أو انقطع Cron Trigger لأي سبب، أو تأخر تشغيله عن منتصف
  // الليل UTC بدقائق، يبقى مستند matches_daily/{غداً} غير موجود في
  // Firestore، فتظهر شاشة "غداً" فارغة تماماً ("لا توجد مباريات مسجّلة")
  // رغم وجود مباريات فعلية — وهذا بالضبط ما كان يحصل. جلبها كل ساعة
  // (بدل مرة واحدة) يضمن أنها معبأة دائماً من أول تشغيل للووركر.
  try {
    await fetchAndStoreFixtures(env, dateKeyOffset(1));
  } catch (_) {
    // فشل جلب "غداً" في تشغيلة معينة لا يوقف بقية المزامنة — ستُعاد
    // المحاولة تلقائياً خلال الساعة التالية بما إنها تُجلب كل ساعة الآن.
  }
